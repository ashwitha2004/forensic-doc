...
import {
  ...
  Camera, Globe, Timer, Shield, Plus, ExternalLink,
} from "lucide-react";
...
export default function ResumeDashboardPage() {
  ...
  const [newLinkSlug,   setNewLinkSlug]   = useState<string | null>(null);
  ...
  return (
    <div className="space-y-6 text-white">
      ...
      {/* ── New link panel ────────────────────────────────────────────────────── */}
      {newLinkUrl && (
        <div className="bg-slate-900 border border-green-700/40 rounded-2xl p-4">
          <p className="text-xs text-green-400 font-semibold mb-2 flex items-center gap-1">
            <CheckCircle className="w-3.5 h-3.5" /> New Share Link Generated
          </p>
          <div className="flex items-center gap-2 bg-slate-950/60 rounded-lg px-3 py-2 mb-2">
            <code className="flex-1 text-xs text-cyan-300 truncate font-mono">
              {newLinkSlug
                ? `${window.location.origin}/r/${newLinkSlug}`
                : `${window.location.origin}${newLinkUrl}`}
            </code>
            <button
              onClick={() => {
                const url = newLinkSlug
                  ? `${window.location.origin}/r/${newLinkSlug}`
                  : `${window.location.origin}${newLinkUrl}`;
            ...
            <a
              href={newLinkSlug ? `/r/${newLinkSlug}` : newLinkUrl}
              target="_blank"
              rel="noopener noreferrer"
              className="shrink-0 p-1 hover:bg-slate-700 rounded transition-colors text-slate-400 hover:text-cyan-400"
            >
              <ExternalLink className="w-4 h-4" />
            </a>
          </div>
          <p className="text-xs text-slate-500">
            All existing tracking, masking, and approval flows apply to this link.
          </p>
        </div>
      )}
